...
release:
	...
	@rm -rf $(BUILDDIR)
	@mkdir -p $(BUILDDIR)
	@mkdir -p $(BUILDDIR)/usr/bin $(BUILDDIR)/lua $(BUILDDIR)/usr/share/a8sidecar
	@cp $(BINDIR)/$(APP_NAME) $(BUILDDIR)/usr/bin/
	@cp README.md LICENSE $(BUILDDIR)/usr/share/a8sidecar/
	@cp $(CONFIGDIR)/filebeat.yml $(BUILDDIR)/usr/share/a8sidecar/
	@cp $(CONFIGDIR)/splib.lua $(BUILDDIR)/lua/
	@mkdir -p $(RELEASEDIR)
	@cd $(BUILDDIR) && tar -czf ../$(RELEASEDIR)/$(RELEASE_NAME).tar.gz * && cd ..
